...
st.set_page_config(page_title="Car Color Detection", page_icon="", layout="wide")
st.title(" Car Color Detection")

# Load model (path hidden from UI)
MODEL_PATH = "Motolite Car Color SAM3(New Model).pt"  # Replace with your actual .pt filename
...
if uploaded_video is not None:
    ...
    # Summary stats — shows each detected color and its count
    st.subheader("Detected Car Colors")
    ...
    with open(output_path, "rb") as f:
        st.download_button(
            ...
            data=f,
            file_name="car_color_detection_output.mp4",
            mime="video/mp4",
        )